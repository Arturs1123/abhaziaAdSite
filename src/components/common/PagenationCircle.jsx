...
const PagenationCircle = ({ nowPage, totalCount, stepValue }) => {
    ...
    return (
        <div className="flex justify-center items-center text-center gap-6 text-[30px] font-semibold">
            {isZero && <img src={'/icon/ArrowLeft.png'} width={48} height={48} className=" cursor-pointer"
            ...
            {isFull && <img src={'/icon/ArrowRight.png'} width={48} height={48} className=" cursor-pointer"
                onClick={() => handleArrowClick(+1)} />
            }
        </div>
    );
